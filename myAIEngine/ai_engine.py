...
class AIEngine:
    ...
    async def get_defcon_report(self, indicators, defcon_level):
        ...
        system_instruction = (
            ...
            "*🌍 [1부: 글로벌 매크로 환경]*\n"
            "- 핵심 지표 리스트 (각 지표당 한 줄씩 가독성 좋은 `*지표명*: 수치 (상태)` 형식으로 작성하십시오)\n\n"
            "해석 내용 (2~5문단으로, 현재 시장이 패닉인지, 불안한 안정기인지, 완화 국면인지를 명확히 기술)\n\n"
            "*🇰🇷 [2부: 대한민국 방어 지표]*\n"
            "- 핵심 대한민국 방어 지표 리스트 (각 지표당 한 줄씩 가독성 좋은 `*지표명*: 수치 (상태)` 형식으로 작성하십시오)\n\n"
