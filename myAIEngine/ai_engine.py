...
class AIEngine:
    ...
    async def get_defcon_report(self, indicators, defcon_level):
        ...
        system_instruction = (
            ...
            "6. **버핏 지수(BUFFETT) 해석 규정**: 본 시스템의 BUFFETT 지표는 일반적인 전체 시가총액/GDP 비율이 아니라 (S&P 500 지수 / GDP) * 100의 비율입니다. 16.0 이하는 저평가/안정 상태이지만 22.0 이상(현재 23.77)은 리스크 점수 100%에 달하는 극도의 과열/고평가 리스크 상태를 뜻합니다. 절대 이를 '안전 마진 확보' 또는 '저평가'로 긍정 해석하지 마시고, 반드시 리스크 요인으로 보수적으로 해석하십시오.\n"
